# Codestra service API contract: Loki

This repository owns the **central-log-authority** for the Codestra observability, analytics, telemetry, and secrets suite.

## Communication rule

Loki keeps its native API and protocol. The shared Codestra control plane in `appolon1908-hue/Codestra-Telemetry` performs only sanitized health, readiness, contract, topology, and immutable-release read-back. It never proxies native query bodies, ingestion, alert delivery, dashboard mutations, secret values, or credential issuance.
...
| `GET` | `/loki/api/v1/query` | query | read_only | never proxied by the Codestra control API |
| `GET` | `/loki/api/v1/query_range` | query | read_only | never proxied by the Codestra control API |
| `GET` | `/loki/api/v1/labels` | query | read_only | never proxied by the Codestra control API |
| `POST` | `/loki/api/v1/push` | ingest | ingest | never proxied by the Codestra control API |

## Suite integrations

| Peer | Direction | Signal | Protocol | Purpose |
|---|---|---|---|---|
| `opentelemetry` | inbound | `logs` | `otlp-http` | receive normalized application logs |
| `alloy` | inbound | `logs` | `loki-push-api` | receive sanitized host and service logs |
| `grafana` | inbound | `logs` | `loki-http-api` | serve business-isolated log queries |
